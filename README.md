...
**치지직(치지직) 오픈소스 로컬 웹 방송 다운로더**

- 웹 UI (브라우저)로 동작 — 설치형 데스크톱 GUI 없음
- 채널 등록 → 방송 시작 자동 감지 → 자동 녹화
- **중단 안전**: 어떤 이유로든 녹화가 끊겨도 `.ts` 세그먼트 + `.m3u8` 로 마지막까지 재생 가능
- **분할 저장**: 특정 시간(예: 30분)마다 chunk_001/, chunk_002/ 로 자동 분할 + 통합 playlist 제공
- 완전 로컬 & 오픈소스 (FastAPI + 순수 세그먼트 다운로더)
...
1. **채널 등록**
   - 채널 URL 또는 32자 hex 채널 ID 입력 (예: `affa78deac0b23d2046b8ed4856c1e62`)
   - 화질 선택 (best / 1080p / 720p ...)
   - 분할 간격 설정 (0 = 분할 없음, 30 = 30분마다 새 chunk)
